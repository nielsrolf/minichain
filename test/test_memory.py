...
def test_question_embedding_memory():
    memory = SemanticParagraphMemory(use_keywords_search=False, use_content_scan_search=False)
...
def test_keyword_memory():
    memory = SemanticParagraphMemory(use_vector_search=False, use_content_scan_search=False)
    text = markdown_browser(url)
    memory.ingest(text, url)
