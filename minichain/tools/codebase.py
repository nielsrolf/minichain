...
from minichain.tools.recursive_summarizer import text_scan, long_document_qa
from minichain.utils.generate_docs import summarize_python_file, get_symbols
...
class Codebase():
    def __init__(self, root_dir=".", extensions=[".py", ".js", ".ts", "README.md"], ignore_files=[".git/", ".vscode/", "__pycache__/", "node_modules/", "dist/", "build/", "venv/", "env/"]):
        ...
        for root, dirs, filenames in os.walk(root_dir):
            for _filename in filenames:
                ...
                for extension in self.extensions:
                    if not any([ignore_file in filename for ignore_file in self.ignore_files]) and filename.endswith(extension):
    ...
    def get_file_summary(self, path):
        if path.endswith(".py"):
            ...
            with open(path, "r") as f:
                text = f.read()
            summary = long_document_qa(text=text, question="Summarize the following file in order to brief a coworker on this project. Be very concise, and cite important info such as types, function names, and variable names of important sections.")
        return f"# {path}\n{summary}\n\n"
    ...
    def edit(self, path, start, end, code):
        ...
        with open(path, "w") as f:
            f.write("\n".join(lines))
        updated_in_context = self.view(path, start - 4, start + len(code.split("\n")) + 4, with_line_numbers=True)
    ...
    def truncate_updated(self, updated_in_context):
        if len(updated_in_context.split("\n")) > 20:
            ...
            updated_in_context = updated_in_context.split("\n")[:9] + ["..."] + updated_in_context.split("\n")[-9:]
        ...
        for symbol in all_symbols:
            if symbol['id'] == symbol_id:
                ...
                with open(symbol['path'], "w") as f:
                    f.write("\n".join(lines))
                updated_in_context = self.view(symbol['path'], symbol['start_line'] - 4, symbol['start_line'] + len(code.split("\n")) + 4, with_line_numbers=True)
                return self.truncate_updated(updated_in_context)
        if is_new:
    ...
    def show_symbol(self, file, symbol_id):
        ...
        for symbol in all_symbols:
            if symbol.id == symbol_id:
                return self.view(symbol.path, symbol.start_line, symbol.end_line, with_line_numbers=True)
