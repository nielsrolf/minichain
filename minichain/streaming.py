...
class Stream:
    ...
    def __init__(
        ...
        self.conversation_stack = [
            i
            for n, i in enumerate(conversation_stack)
            if i not in conversation_stack[:n]
        ]
    ...
    def __exit__(self, type, value, traceback):
        ...
        if len(self.conversation_stack) > 0:
            self.conversation_stack = self.conversation_stack[:-1]
    ...
    async def to(self, history, role, **kwargs):
        ...
        try:
            ...
            current_message = message_type(
                content="", conversation_id=conv_id, **kwargs
            ).dict()
